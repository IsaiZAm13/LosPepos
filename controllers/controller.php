...
class MvcController{
	...
	public static function registroUsuarioController(){
				...
				if(isset($_POST["usuarioRegistro"])){
					...
					if(preg_match('/^[a-zA-Z0-9]*$/', $_POST["usuarioRegistro"]) &&
					   preg_match('/^[a-zA-Z\sñáéíóúÁÉÍÓÚ]+$/', $_POST["nombresRegistro"]) &&
					   preg_match('/^[a-zA-Z0-9]*$/', $_POST["apellidoPaternoRegistro"]) &&
					   preg_match('/^[a-zA-Z0-9]*$/', $_POST["apellidoMaternoRegistro"]) &&
				...
				$datosController = array( "usuario"=>$_POST["usuarioRegistro"],
									      ...
									      "apellidoPaterno"=>$_POST["apellidoPaternoRegistro"],
										  "apellidoMaterno"=>$_POST["apellidoMaternoRegistro"],
					                      "password"=>$encriptar,
	...
	public static function editarUsuarioController(){
		...
		echo '<input type="hidden" value="'.$respuesta["id"].'"name="idEditar">
		      ...
		      <input type="text" value="'.$respuesta["nombres"].'" name="nombresEditar" required>

		      <input type="text" value="'.$respuesta["apellidoPaterno"].'" name="apellidoPaternoEditar" required>

		      <input type="text" value="'.$respuesta["apellidoMaterno"].'" name="apellidoMaternoEditar" required>
	...
	public static function actualizarUsuarioController(){
		...
		if(isset($_POST["usuarioEditar"])){
			...
			if(preg_match('/^[a-zA-Z0-9]*$/', $_POST["usuarioEditar"]) &&
					   preg_match('/^[a-zA-Z\sñáéíóúÁÉÍÓÚ]+$/', $_POST["nombresEditar"]) &&
					   preg_match('/^[a-zA-Z0-9]*$/', $_POST["apellidoPaternoEditar"]) &&
					   preg_match('/^[a-zA-Z0-9]*$/', $_POST["apellidoMaternoEditar"]) &&
